    ...
    for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
            if (node.tagName === 'IFRAME') {
				console.log('偵測到新的 iframe:', node);
				oriprint=node.contentWindow.print;
				node.contentWindow.print = () => {
					try {
						const win = node.contentWindow;
						const doc = node.contentDocument;
						let alldivs=doc.body.children[0].children;
						...
						let hrarea=node.contentDocument.querySelector("body > div > div.page > div:nth-child(2)");
						if (hrarea.style.display=='flex'){
							hrarea.className="page";
						}
						oriprint();
					} catch (e){
						console.log('不使用');
...
//WSS
socket = new WebSocket("wss://localhost:6690/PHPCIISservice");
socket.onmessage = (event) => {
	...
	if (data.trace_type=="hisGetCriticalIllness"){
		if (data.content.ret==0){
				let rettext=base64decode(data.content.pBuffer);
				let res=""
				if (rettext.trim()!=""){
					res=rettext.trim();
				}
			...
			let codelist={
				...
				4050:"安全模組尚未與IDC認證" ,
				5002:"卡片已註銷" ,
				5102:"使用者所輸入之pin值，與卡上之pin值不合" ,
				5109:"密碼輸入過程按『取消』鍵" ,
				5141:"未置入醫事人員卡，僅能讀取重大傷病有效起訖日期 " ,
				9100:"藥師，藥劑生，特約藥局或藥局專用卡無權限" 
			}
			if (Object.keys(codelist).includes(data.content.ret.toString())){
				let rettext=base64decode(data.content.pBuffer);
				let res=""
				if (rettext.trim()!=""){
					res=rettext.trim();
				...
				alert(codelist[data.content.ret]+"\n"+res)
			} else {
				alert('讀卡異常')
			}
		}
...
function createchest(){
	...
	d10='https://phpcis.chshb.gov.tw/prescriptBatchComplete';
	...
	function create_OPD_list(){
		...
		button_checkcritical = document.createElement('button');
		button_checkcritical.textContent='查重大';
		button_checkcritical.style.width = '78px';
		button_checkcritical.style.left = '248px';
		button_checkcritical.style.top = '2px';
		button_checkcritical.style.position = 'absolute';
		button_checkcritical.addEventListener('click', button_checkcritical_handle);
		myDraggable.appendChild(button_checkcritical);
		...
		button_autocompletev2.style.left = '328px';
		...
		button_autocompletesingle.style.left = '490px';
		...
		button_changenumber.style.left = '652px';
		...
		button_listdelete.style.left = '734px';
		...
		myDraggable.style.width='819px';
		myDraggable.style.left=window.innerWidth-819 +'px';
	}
	...
	function create_FM(){
		...
		button_FMdebug.style.width = '100px';
		button_FMdebug.style.height = '36px';
		...
		button_FMdgen = document.createElement('button');
		button_FMdgen.textContent='產出D組';
		button_FMdgen.style.width = '100px';
		button_FMdgen.style.height = '36px';
		button_FMdgen.style.left = '102px';
		button_FMdgen.style.top = '2px';
		button_FMdgen.style.position = 'absolute';
		button_FMdgen.addEventListener('click', button_FMdgen_handle);
		myDraggable.appendChild(button_FMdgen);
		myDraggable.style.width='204px';
		myDraggable.style.height = 40+'px';
		myDraggable.style.left=window.innerWidth-204-borderx +'px';
	}
	...
	function create_add6Z(){
		create_myDraggable();
		button_add6z = document.createElement('button');
		button_add6z.textContent='添加6Z選項';
		button_add6z.style.width = '160px';
		button_add6z.style.height = '70px';
		button_add6z.style.left = '2px';
		button_add6z.style.top = '2px';
		button_add6z.style.position = 'absolute';
		button_add6z.addEventListener('click', button_add6z_handle);
		myDraggable.appendChild(button_add6z);
		
		myDraggable.style.width='160px';
		myDraggable.style.height = 74+'px';
		myDraggable.style.left=window.innerWidth-700-borderx +'px';
	}
	function button_add6z_handle(){
		let sel=document.querySelector("#root > div.wrapper > main > div > div.line > form > div:nth-child(2) > div > div > div > div > select");
		if (!sel.innerHTML.includes("6Z 中老年預注")){
			let opt=document.createElement("option");
			opt.value="21";
			opt.textContent="6Z 中老年預注";
			sel.appendChild(opt);
			opt.selected=true;
	...
	function button_changenewHE_handle(){
		function getlistfromtxt(){
			...
			for (let i=0;i<atxt.length;i++){
				...
				let drid=line.substring(185-dif,195-dif);
				let tempobj={};
			...
			let remin=Math.floor(rec*3/60);
			let resec=Math.floor(rec*3%60);
			...
			if (cont){
				...
				for (let i=0;i<hbcv.length;i++){
					let line=hbcv[i];
					let dif=line.replace(/[^\x00-\x7F]/g, "aa").length-line.length;
					let rline=line.substring(0,195-dif);
					retres+=rline+"\r\n";
				}
		...
		function genprea(xhr,pat,preventionDataId){
			...
			let cynow=new Date().toISOString().split("T")[0];
			let item_10=cytomky(cynow);
				  ...
				  .then((response) => {
					...
					fetch(furl3, {
					  ...
					  .then((response) => {
						...
						let patid=document.querySelector("#root > div.wrapper > main > div > form > div:nth-child(1) > div:nth-child(2) > div").textContent;
						let noturl="https://phpcis.chshb.gov.tw/api/v1/notifications/check";
						xhr.open( 'POST', noturl, false );
						let postjson={
							"action": patid,
							"eventType": 2
						}
						...
						let notjson=JSON.parse(xhr.responseText).result;
						let notmsg="";
						for (let i=0;i<notjson.length;i++){
							let notitem=notjson[i];
							let msg1=notitem.promptWindowContent1;
							let msgc=notitem.notificationGroupName;
							let msg2=notitem.promptWindowContent2;
							notmsg=notmsg+(i+1)+". "+msg1+" "+msgc+" "+msg2+"\n";
						}
						if (notmsg!=""){
							alert(notmsg);
						}
	...
	function button_showdata_handle(){
		...
		let registrationId=ccc.split("/")[ccc.split("/").length-1];
		let xhr = new XMLHttpRequest();
		...
		let he1js={
			...
			"CRE_S":"09015C",
			"eGFR":"Y00001",
			"urineProteinReference":"09040C001",
			"SGOT":"09025C",
			"SGPT":"09026C",
		}
		*/
...
<html lang="zh-Hant">
  ...
  "</html>"
    ...
    <body>
		...
		console.log(retobj);
		let newWin = window.open('', '_blank',"width=1200,height=1000");
		newWin.document.open();
	...
	function button_checkcritical_handle(){
		let uuid=crypto.randomUUID();
		let message = {
			command: "hisGetCriticalIllness",
			content: null,
			timestamp: new Date().toISOString(),
			trace_id: uuid,
			trace_type: "hisGetCriticalIllness"
		};
	...
	async function button_FMdgen_handle() {
	let xmlHttp = new XMLHttpRequest();
	let csvopt = {
		types: [
			{
	...
	let handles = await showOpenFilePicker(csvopt);
	let file = await handles[0].getFile();
	let csvar = await file.arrayBuffer();
	let csvcon = new TextDecoder('big5').decode(csvar);
	csvcon = csvcon.replaceAll('\'', '');
	csvcon = csvcon.replaceAll('\"', '');
	let a_csvcon = csvcon.split('\r\n');
	let idlist = [];
	for (let i = 0; i < a_csvcon.length; i++) {
		let line = a_csvcon[i];
		let aline = line.split(",");  
		if (aline[1] && aline[1].length == 10) {
			idlist.push(aline[1]);
		}
	}
	if (idlist.length==0){
	...
	let basicurl="https://phpcis.chshb.gov.tw/api/v1/clinics/settings/find";
	xmlHttp.open('GET', basicurl, false );
	xmlHttp.send();
	let res_basic=xmlHttp.responseText;
	let json_basic=JSON.parse(res_basic);
	let areaCode=json_basic.result.areaCode;
	let clinicCode=json_basic.result.clinicCode;
	let doctorId=json_basic.result.doctorId;
	
	let exportdata=[];
	for (let i=0;i<idlist.length;i++){
		let personalId=idlist[i];
		let findurl="https://phpcis.chshb.gov.tw/api/v1/personal_infos/list_with_registration?search=false&personalId="+personalId;
		xmlHttp.open('GET', findurl, false );
		xmlHttp.send();
		let res_find=xmlHttp.responseText;
		let json_find=JSON.parse(res_find);
		if (json_find.result.length==1){
			let personalInfoId=json_find.result[0].personalInfoId;
			let detailurl="https://phpcis.chshb.gov.tw/api/v1/personal_infos/find";
			xmlHttp.open('POST', detailurl, false );
			let payload={
				"isNHICard": false,
			...
			let res_detail=xmlHttp.responseText;
			let json_detail=JSON.parse(res_detail);
			let c=json_detail.result;
			let temp=[];
			temp.push("A");
			temp.push("16");
			temp.push(areaCode);
			temp.push(clinicCode);
			temp.push(c.personalId);
			temp.push(c.birthday.replaceAll("-",""));
			temp.push(c.name);
			temp.push(c.gender);
			temp.push(c.residentAddress);
			if (c.mobile!="" && c.mobile!="0911111111"){
				temp.push(c.mobile);
			} else if (c.phone1!=""){
				temp.push(c.phone1);
			} else if (c.phone2!=""){
			...
			temp.push(doctorId);
			temp.push("D");
			let cynow=new Date().toISOString().split("T")[0].replaceAll("-","");
			temp.push(cynow);
			temp.push("");
			temp.push("");
	...
	exportFixedWidthFile(exportdata,areaCode,clinicCode);
	...
	function padString(str, targetLength) {
		let len = getDisplayLength(str);
		if (len > targetLength) {
			let result = '';
			let accLen = 0;
			for (let ch of str) {
				let charLen = ch.charCodeAt(0) > 127 ? 2 : 1;
				if (accLen + charLen > targetLength) break;
				result += ch;
				accLen += charLen;
			}
			return result;
	...
	async function exportFixedWidthFile(data,areaCode,clinicCode) {
		const fieldWidths = [	1, 
								2, 
								1,
								10,
		...
		const lines = data.map(row => {
			return row.map((col, i) => padString(col, fieldWidths[i])).join('');
		});
		...
		let mnow=new Date().toISOString().split("T")[0].split("-")[1];
		a.download = `${areaCode}${clinicCode}${mnow}01FM.txt`;
		a.click();
		alert("下載完成後，要再做3件事才可以上傳\n1. 修改文字檔FM前的兩個流水碼\n2. 另存新檔，編碼方式選擇ANSI\n3.重新打開文字檔，檢查中文部分及地址後面的電話是否正確，有些因編碼問題會導致長度不對，補上空白即可");
	}
}
...
function createmyDraggable_3(){
	...
	var line4 = document.createElement('table');
	line4.id='line4';
	line4.style.width = "440px";
	line4.style.tableLayout = "fixed";
			...
			if (json_orderdetail.result.diseases.length>0){
				if (json_orderdetail.result.FreqOrderPrescription.length>0){
					...
					for (let i=0;i<json_orderdetail.result.FreqOrderPrescription.length;i++){
						...
						if (json_orderdetail.result.FreqOrderPrescription[i].applicationId=="A2051C"){
							isroutinevaccine=true;
						}
					}
					if (confirm('是否將處方帶入個案'+p_name+'，身分別:'+p_visittype+'?')){
						...
						if (isroutinevaccine){
							isroutinevaccine_PE=prompt('理學檢查內容',"")
						}
						autocomplete();
					}
				} else {
	...
	function autocomplete(){
		...
		for (i3=0;i3<array_patient.length;i3++){
			...
			forinjectdate['CC']=json_orderdetail.result.CC;
			forinjectdate['PE']=json_orderdetail.result.PE;
			if (isroutinevaccine && isroutinevaccine_PE){
				forinjectdate['PE']=isroutinevaccine_PE;
			}
			...
			if (jres_registrations.result.caseTypeId==16){
				forinjectdate['caseTypeId']=23;
				forinjectdate['caseTypeCode']='09';
				console.log(jres_registrations.result.caseTypeId)
			} else {
				forinjectdate['caseTypeId']=jres_registrations.result.caseTypeId;
				forinjectdate['caseTypeCode']=jres_registrations.result.caseTypeCode;
			}
			...
			for (i5=0;i5<json_orderdetail.result.FreqOrderPrescription.length;i5++){
				let row=i5+1;
				let selectedpaytype=line4.rows[row].cells[3].children[0].value;
				let totalQty=line4.rows[row].cells[2].children[0].value;
				if (line4.rows[row].cells[0].children[0].checked){
					tempprescription={};
					...
					tempprescription['qty']=json_orderdetail.result.FreqOrderPrescription[i5].qty;
					if (selectedpaytype){
						tempprescription['selfPayType']=selectedpaytype;
					} else {
						selfPayType=json_orderdetail.result.FreqOrderPrescription[i5].selfPayType;
						if (selfPayType=='*' || selfPayType=='#'){
							tempprescription['selfPayType']=json_orderdetail.result.FreqOrderPrescription[i5].selfPayType;
					...
					if (totalQty<1){
						...
						tempprescription['totalQty']=totalQty;
					}
					forinjectdate['prescriptions'].push(tempprescription);
				}
			...
			forinjectdate['weight']=jres_health_records.result.weight*1;
			forinjectdate['writeICCard']=jres_registrations.result.isCardCheck;
			if (forinjectdate['caseTypeId']==16){
				forinjectdate['caseTypeId']=23;
			}
			forinjectdates=JSON.stringify(forinjectdate);
			url_check_valid='https://phpcis.chshb.gov.tw/api/v1/health_records/check_valid';
	...
	function ordercombo_change(){
		...
		line4.appendChild(tr);
		let cell=document.createElement("th");
		let title=["勾","序","數量","自付","名稱"];
		let titlewidth=["20","40","55","105","210"];
		for (let o=0;o<title.length;o++){
			let tiname=title[o];
			cell=document.createElement("th");
			cell.textContent=tiname;
			cell.style.textAlign = "center";
			cell.style.width=titlewidth[o]+"px";
			tr.appendChild(cell);
		}
		let paytype=[
			" 請選擇",
		...
		if (json_orderdetail.result.FreqOrderPrescription.length>0){
			for (let i6=0;i6<json_orderdetail.result.FreqOrderPrescription.length;i6++){
				let seorder=json_orderdetail.result.FreqOrderPrescription[i6];
				let ordernum=i6+1;
				let applicationId=seorder.applicationId;
				let selfPayType=seorder.selfPayType;
				let prescriptionName=seorder.prescriptionName;
				let totalQty=seorder.totalQty;
				let prescriptionModel=seorder.prescriptionModel;
				...
				let sebox = document.createElement("input");
				sebox.type = "checkbox";
				sebox.checked=true;
				sebox.addEventListener('click', function(e) {
					changebox(ordernum);
				});
				cell.appendChild(sebox);
				cell=document.createElement("td");
				cell.textContent=ordernum;
				cell.addEventListener('click', function(e) {
					changebox(ordernum);
				});
				cell.style.textAlign = "center";
				...
				cell=document.createElement("td");
				tr.appendChild(cell);
				let numedit = document.createElement("input");
				numedit.style.width="50px";
				numedit.type = "number";
				numedit.style.textAlign = "center";
				numedit.min = 1;
				numedit.max = 99;
				numedit.step = 1;
				if (totalQty>0){
					numedit.value=totalQty;
				} else {
					numedit.value=1;
				}
				if (prescriptionModel=="Drug"){
					numedit.disabled=true;
				}
				
				cell.appendChild(numedit);
				cell=document.createElement("td");
				tr.appendChild(cell);
				let payselect = document.createElement("select");
				cell.appendChild(payselect);
				let option = document.createElement("option");
				//option.value="";
				//option.text="請選擇";
				//payselect.appendChild(option);
				for (let kkk=0;kkk<paytype.length;kkk++) {
					let payitem=paytype[kkk];
					let value=payitem.split(" ")[0];
					option = document.createElement("option");
					option.value=value;
					option.text=payitem;
					if (selfPayType==value){
						option.selected=true;
					}
					payselect.appendChild(option);
				}
				cell=document.createElement("td");
				cell.textContent=prescriptionName;
				cell.style.whiteSpace = "nowrap";
				cell.style.overflow = "hidden";
				cell.style.textOverflow = "ellipsis";
				cell.title=applicationId+"\n"+prescriptionName;
	...
	function changebox(row){
		let chbo=line4.rows[row].cells[0].children[0];
		if (chbo.checked){
			chbo.checked=false;
		} else {
			chbo.checked=true;
...
function grabdata(){
	...
	for (let i=1;i<fd2.length;i++){
		if (document.querySelector("#uncontrolled-tab-example-tabpane-tab1 > div:nth-child(7) > input[type=checkbox]:nth-child("+(2*i-1)+")").checked){
			fd=fd+fd2[i]+",";
		}
	}
	if (fd!=""){
