...
async function queryalldata(){
	...
	if (ccc.includes(ddd)){
		...
		const url_list={
			...
			"admission": "https://medcloud2.nhi.gov.tw/imu/api/imue0070/imue0070s02/get-data?cli_datetime=",
		}
		const headers = {
...
async function postData(url,payload) {
  try {
    ...
    const result = await response.json();  // 解析回傳 JSON
    return result
  } catch (err) {
    return false
  }
	...
	htmls=String.raw`
		...
		<body>
			...
			<div class="titlearea">
				...
				<a href="javascript:showtable(5)" class="mainmenuitem">住院紀錄</a>
			...
			<div id="div_admission" style="display: none;"></div>
		</body>
		<script>
			function showtable(num){
				...
				let f=document.getElementById('div_admission');
				let arr=[a,b,c,d,e,f]
			...
			window.onload=function(){
				...
				gen_admission('div_admission',vpndata);
			}
			function gen_admission(divid,vpndata){
				let div=document.getElementById(divid);
				div.innerHTML="";
				let fdiv=additem('div','','',div);
				fdiv.style.display='flex'
				fdiv.style.setProperty("align-items", "flex-start");
				let tableadmission=additem('table','table_image','table_admission',fdiv);
				let titlerow=tableadmission.insertRow();
				let titlelist=['醫療院所','主診斷','住院日期','住院紀錄'];
				for (let i=0;i<titlelist.length;i++){
					titlerow.insertCell().textContent=titlelist[i]
				}
				for (let i=0;i<vpndata.admission.robject.length;i++){
					let r=vpndata.admission.robject[i];
					let row=tableadmission.insertRow();
					let cell=row.insertCell();
					cell.textContent=r.hosp;
					cell=row.insertCell();
					icd_cname="";
					if (r.icd_cname){
						icd_cname=r.icd_cname;
					}
					icd_code="";
					if (r.icd_code){
						icd_code=r.icd_code;
					}
					cell.innerHTML=icd_cname+"<br>"+icd_code;
					cell=row.insertCell();
					in_date="無";
					if (r.in_date){
						in_date=r.in_date.split("T")[0];
					}
					out_date="無";
					if (r.out_date){
						out_date=r.out_date.split("T")[0];
					}
					cell.innerHTML="住院日期: "+in_date+"<br>出院日期: "+out_date;
					cell=row.insertCell();
					let admissionbutton=additem('button','admissionbutton','admissionbutton',cell);
					admissionbutton.textContent="出院病摘";
					admissionbutton.addEventListener('click',function () {
						if (Object.keys(vpndata.admissionxml).includes(r.mds_pdf_file)){
							let newWindow = window.open("", "_blank", "width=1600,height=900");
							newWindow.document.write(vpndata.admissionxml[r.mds_pdf_file].html);
				...
				for (let i=0;i<codelist.length;i++){
					...
					for (let j=0;j<data.maxbycodelength;j++){
						if (j<datelist.length){
							...
							if (d.drug_left>7){
								temc.style.background="#FF5151";
							} else if (d.drug_left>0){
						...
						for (k=0;k<diaglist.length;k++){
							...
							for (let l=0;l<data.maxbydatelength;l++){
								if (l<drugarray.length){
									...
									additem('br','','',temc);
									additem('span','','',temc).textContent=d.drug_atc3_code;
									additem('br','','',temc);
			...
			`
	newWindow = window.open("", "_blank", "width=1600,height=900");
